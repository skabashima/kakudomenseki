...
## コース(角度編 19 + 面積編 40 = 全 59 ステージ)

| 編 | 内容 | ステージ例 |
|---|---|---|
| **角度編** | 角度の問題だけを進む | 三角形の角、時計の針、正多角形と対角線、ブーメラン形、折り返し、星形、円周角、弧の比、三角形の五心、三角方程式、ベクトルのなす角、2 直線のなす角、空間のなす角 |
| **面積編** | 長さ・面積・体積の問題を進む | 三角形の面積、道の面積、長方形の中の点、葉っぱ形、三平方、相似比、立体の体積と表面積、水そうと水位、図形の移動、角錐と円錐、空間の三平方、ヒポクラテスの月、方べきの定理、チェバとメネラウス、二等分線と中線定理、余弦定理、ヘロン、点と直線の距離、円の方程式、空間図形の体積、6分の1公式、回転体と曲線 |
...
高校受験レベルは 平行線と線分の比・相似比と体積比・角錐と円錐・空間の三平方まで。
ステージの中でも 1 問目 → 3 問目でバリエーションが難しくなる。
無理数が出る問題は「√3 = 1.73 として」のように明記して小数で答えさせる。

問題はプロシージャル生成で、**問題文と答えの組だけ数えても約 25,000 通り**
...
1 ステージあたりの解法は 2〜6 種類(全 59 ステージで約 220 種類)。
...
- **購入で解放**: 残り 51 ステージ / 全ステージの「挑戦 10問」(王冠) /
