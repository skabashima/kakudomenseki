...
## コース(角度編 19 + 面積編 32 = 全 51 ステージ)

| 編 | 内容 | ステージ例 |
|---|---|---|
| **角度編** | 角度の問題だけを進む | 三角形の角、時計の針、正多角形と対角線、ブーメラン形、折り返し、星形、円周角、弧の比、三角形の五心、三角方程式、ベクトルのなす角、2 直線のなす角、空間のなす角 |
| **面積編** | 長さと面積の問題だけを進む | 三角形の面積、道の面積、長方形の中の点、葉っぱ形、三平方、相似比、ヒポクラテスの月、方べきの定理、チェバとメネラウス、二等分線と中線定理、余弦定理、ヘロン、点と直線の距離、円の方程式、空間図形の体積、6分の1公式、回転体と曲線 |
...
問題はプロシージャル生成で、**問題文と答えの組だけ数えても約 20,000 通り**
...
1 ステージあたりの解法は 2〜6 種類(全 51 ステージで約 190 種類)。
...
- **購入で解放**: 残り 43 ステージ / 全ステージの「挑戦 10問」(王冠) /
