...
第1章「三角形の秘密」(ギリシャ、紀元前300年ごろ):

1. 導入(親方に「三角形の内角の和はいつも同じ」と言われる)
2. **測る**: 三角形の頂点を指で動かすと 3 つの角がその場で変わる。
   形をいくつか作って記録し、「何が一定か」を選ぶ。
   **3 回記録するまで選択肢は出ない**(当てずっぽうで進ませない)
3. なぜ 180° か(平行線と錯角の図で見せる)
4. 見つけたことを使って本編と同じ生成器の問題を 1 問(3 択)
5. 締め ― 次の謎へ

会話の場面には挿絵をつけている(`scenes/story_art.gd`)。**画像ファイルは持たず
その場で図形として描く**ので、どの端末でもぼやけず、アプリの容量も増えない
(効果音やアイコンと同じ方針)。第1章は「三角形の畑と測量の見習い」「夕暮れの砂に
描いた四角形」の 2 枚。

- データは `core/story_defs.gd`、画面は `scenes/story.gd`。章を足すときは
  `CHAPTERS` に足し、動かす図が新しいなら `spec_of` に足す
- 図の点をつまんで動かす仕組みは `ui/figure_view.gd` の `drag_points` /
  `point_dragged`(本編では使わないので既存の動作に影響しない)
- 角度は**整数に丸めても和が 180 になるように誤差を分配**している。
  ふつうに四捨五入すると 40 + 12 + 129 = 181 と見えて、発見そのものが濁るため
- `tests/story_check.gd` が章データの不備と、丸めの和が 180 になることを検証する
...
core/
  ...
  story_defs.gd       ストーリーの章データと、動かす図のスペック
...
scenes/
  ...
  story.gd/.tscn            ストーリー(図を動かして不変量を見つける)
