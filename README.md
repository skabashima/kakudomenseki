...
問題はプロシージャル生成で、**問題文と答えの組だけ数えても約 9,800 通り**
(`tests/variation_report.tscn` で実測。図の数値の違いまで含めるとさらに多い)。
どのステージも数十通り以上のバリエーションがあり、周回・チャレンジで同じ問題ばかり出ることはない。
バリエーションには難度 tier があり(0〜2 = 通常ステージの 1〜3 問目、
3〜4 = 挑戦モード後半とチャレンジ終盤にだけ出る高難度)、
挑戦モードでは 10 問かけて段階的に難しくなる。
...
- **挑戦モード(単元別チャレンジ)**: クリアした単元には「挑戦 10問」が出現。
  ハート 3 つで高難度バリエーション(ジグザグの平行線、二重外角、おうぎ形の
  まわりの長さ、十字形、長方形の対角線、面積からの角度逆算 など)を 10 問連続。
