...
| 3 | 円のふしぎ | 円の大きさ | 円周 ÷ 直径 = 3.14…(π) |
| 4 | 高さが同じなら | 頂点を横に滑らせる | 底辺と高さが同じなら面積は同じ(等積変形) |
| 5 | 水は量で決まる | 水そうの幅 | 幅 × 深さ は水の量そのもの |
| 6 | 影で測る | 太陽の向き | 同じ時刻なら 高さ ÷ 影 はどの物でも同じ |
| 7 | 通せんぼの板 | 板の進んだ長さ | 重なりの面積 ÷ 進んだ長さ は一定(たて) |
| 8 | 直角の宝 | 直角をはさむ 2 辺 | a² + b² = c²(三平方) |
| 9 | 円周角のふしぎ | 円周上の席 | 円周角は中心角の半分 |
| 10 | 大きくすると | 相似比 | 面積比は相似比の 2 乗 |
| 11 | 倉の梁 | おくゆきと高さ | 対角線² = 3 辺の平方の和(三平方を 2 回) |
| 12 | 二倍の樽 | 拡大の倍率 | 体積比は相似比の 3 乗 |
| 13 | 三角帽子の型紙 | 母線の長さ | ひらいた弧 = 底面の円周(中心角 = 360 × r ÷ l) |
| 14 | 池ごしの板 | 渡す向き | 近い方 × 遠い方 は一定(方べきの定理) |
| 15 | 一点で交わる三本の綱 | 交点の位置 | 三つの辺の比をかけると 1(チェバの定理) |
| 16 | 外接円のひみつ | 円周上の頂点 | a ÷ sin A = 2R(正弦定理) |
| 17 | はさむ角と面積 | 2 辺のはさむ角 | 面積 = ½ × a × b × sin C |
| 18 | 放物線が囲む | 水面(直線)の高さ | 面積 = (交点の差)³ ÷ 6 |
...
- `tests/story_check.gd` が全章のデータと、**「一定」と言っている量が本当に
  ...
  検証する
- `tests/story_play.tscn` が**全18章を実際に最後まで通してプレイする**。各場面で
...
core/
  ...
  story_figs2.gd      同上(第11章から。水そう・影・移動・立体・方べき・チェバ)
...
scenes/
  ...
  story_select.gd/.tscn     ストーリーの章えらび(全18章・前の章で解放)
...
tests/
  ...
  story_play.tscn        ストーリー全18章を実際に最後まで通す (--headless 可)
